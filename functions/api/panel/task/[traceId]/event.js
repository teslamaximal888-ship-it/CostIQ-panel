import { appendLifecycleEvent, makeLifecycleEvent } from "../../_shared/task-lifecycle.js";
...
export async function onRequestPost({ request, env, params }) {
  ...
  const event = makeLifecycleEvent(cleanText(payload.type, 80) || "operator_event", now, {
    by: cleanText(payload.by, 80) || "bridge",
    source: cleanText(payload.source, 100) || "event_api",
    message: cleanText(payload.message, 1600),
    note: cleanText(payload.note, 1600),
  });
  const updatedTask = {
    ...task,
    lifecycle: appendLifecycleEvent(task, event),
    updated_at: now,
  };
  ...
  return jsonResponse({ ok: true, event, events: taskEventLogSnapshot(updatedTask, 40), task: updatedTask });
}
