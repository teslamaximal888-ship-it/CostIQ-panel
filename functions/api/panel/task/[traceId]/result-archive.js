...
function contentDisposition(fileName) {
  const original = cleanText(fileName, 180) || "results.zip";
  const fallback = original.replace(/[\\/:*?"<>|#%{}^~[\]`]/g, "_").replace(/[^\x20-\x7E]/g, "_") || "results.zip";
...
function shortResultFileName(task, fileName, fallbackExt = ".zip") {
...
export async function onRequestGet({ request, env, params }) {
  ...
  const archive = task.result_archive || {};
  if (!archive.key) {
    return jsonResponse({ ok: false, error: "result_archive_not_found" }, 404);
  }
  const object = await bucket.get(archive.key);
  if (!object) {
    return jsonResponse({ ok: false, error: "result_archive_missing" }, 404);
  }

  const headers = new Headers();
  object.writeHttpMetadata(headers);
  headers.set("Content-Type", headers.get("Content-Type") || archive.type || "application/zip");
  headers.set("Content-Disposition", contentDisposition(shortResultFileName(task, archive.name, ".zip")));
