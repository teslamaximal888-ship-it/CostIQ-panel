import { scheduleTaskResume, taskResumeSnapshot } from "../../_shared/task-resume.js";
...
export async function onRequestPost({ request, env, params }) {
  ...
  const scheduled = scheduleTaskResume(task, now, {
    by: cleanText(payload.by, 80) || "admin",
    source: cleanText(payload.source, 100) || "resume_api",
    mode: cleanText(payload.mode, 40) || "resume",
    message: cleanText(payload.message, 1000),
  });
  if (!scheduled.ok) {
    return jsonResponse(scheduled, 409);
  }
  ...
  const updatedTask = {
    ...
    retry_after: cleanText(payload.retry_after, 80),
    error_text: "",
  ...
  return jsonResponse({
    ok: true,
    resume: taskResumeSnapshot(updatedTask),
    checkpoint: taskCheckpointSnapshot(updatedTask),
    events: taskEventLogSnapshot(updatedTask, 40),
    task: updatedTask,
  });
}
