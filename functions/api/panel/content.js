...
function defaultItems() {
  ...
  return [
    ...
    },
    {
      id: "vote-next-tool",
      type: "news",
      title: "Топ используемых навыков",
...
function publicItem(item, userId = "") {
  const base = {
    ...
    event_at: cleanText(item.event_at, 80),
    body: item.body,
...
function itemFromBody(body, existing = null) {
  ...
  const item = {
    ...
    action: cleanText(body.action || body.action_label || (existing && existing.action), 80),
    event_at: cleanText(body.event_at || (existing && existing.event_at), 80),
