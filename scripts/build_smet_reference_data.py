...
KVR_RATES_FILE = DATA_DIR / "kvr_rates.json"
MEDIAN_PRICES_FILE = DATA_DIR / "median_prices.json"
KVR_MATERIALS_FILE = DATA_DIR / "kvr_materials.json"
...
def build_kvr_index(kvr_data: dict[str, Any], medians: list[dict[str, Any]]) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    items = kvr_data.get("items", {}) if isinstance(kvr_data, dict) else {}
    ...
    for code, entry in items.items():
        if not isinstance(entry, dict):
            continue
        code_text = str(entry.get("code") or code or "").strip()
        code_key = normalize_key(code_text)
        best_median = None
        med_rows = median_by_code.get(code_key, [])
        if med_rows:
            best = max(med_rows, key=lambda row: row.get("ot_count") or row.get("count") or 0)
            best_median = {
        ...
        for rate in entry.get("rates", []) or []:
            if not isinstance(rate, dict):
                continue
            desc = normalize_key(rate.get("desc") or rate.get("description"))
            section = normalize_key(rate.get("section"))
            if not desc:
                continue
            index.setdefault((desc, section), {
                "kvr_code": code_text,
                "kvr_name": clean_text(entry.get("name"), 260),
                "kvr_median": best_median,
            })
    return index
...
def build_material_price_index(rates: list[dict[str, Any]], medians: list[dict[str, Any]]) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    for row in rates:
        ...
        if section not in {"материалы", "мтр", "л7"}:
            continue
...
def rate_record(
    ...
    kvr_info = kvr_index.get((normalize_key(title), normalize_key(section)), {})
...
def main() -> None:
    ...
    kvr_rates = load_json(KVR_RATES_FILE, {})
    ...
    searchable_rates = dedupe_latest_rates(source_rates)
    source_gesn = gesn_source.get("norms", []) if isinstance(gesn_source, dict) else []
    kvr_index = build_kvr_index(kvr_rates, medians if isinstance(medians, list) else [])
    ...
    rates = [
        ...
        for index, item in enumerate(searchable_rates, start=1)
    ...
    payload = {
        "version": "smet-reference-panel-v2",
